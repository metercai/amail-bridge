        ...
        while let Ok(event) = rx.recv() {
            let should_rescan = match event.kind {
                EventKind::Create(_) | EventKind::Modify(_) | EventKind::Remove(_) => {
                    // Skip if the event was triggered by our own write_routes_file
                    if router.writing_routes.load(Ordering::SeqCst) {
                        continue;
                    }
                    // Only rescan on relevant files: profile dirs, amail.json,
                    // config.yaml, or the routes config file itself.
