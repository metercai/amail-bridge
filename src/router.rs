...
impl ProfileRouter {
    ...
    pub fn full_scan(&self) {
        ...
        // Write discovered routes to a TOML file for visibility and manual editing
    ...
    fn write_routes_file(&self) {
        let path = self.profiles_dir.parent()
            .map(|p| p.join("amail-bridge-routes.toml"));
        let Some(path) = path else { return };

        let routes = self.routes.read().unwrap_or_else(|e| e.into_inner());
        let mut out = String::from("# Auto-generated by amail-bridge. Edit manually to override.\n\n");
        for (email, route) in routes.iter() {
